...
Class Form_builder {
	...
	public $form_attrs = 'method="post" action="" class="form-inline"'; // form tag attributes
	public $label_colons = FALSE; // add colons to form labels?
	public $textarea_rows = 3; // number of rows for a textarea
	public $textarea_cols = 60; // number of columns for a textarea
	public $text_size_limit = 60; // text size for a text input
	public $submit_value = "Submit"; // submit value  (what the button says)
	public $submit_class = "btn btn-warning"; // submit value  (what the button says)
	...
	public $display_modes = FALSE; // for display on columns
	public $modes = array('column-left', 'column-right'); // for display on columns
	...
	public function initialize($params = array())
	{
		...
		if ($CI->config->item('csrf_protection') === TRUE AND empty($this->key_check))
		{
			...
			$this->key_check = $CI->security->get_csrf_hash();
			$this->key_check_name = $CI->security->get_csrf_token_name();
		}
	}
	 ...
	 * @return	string
	 */
	public function render_divs_original($fields = NULL)
	{
		...
		if (!empty($this->submit_value) AND $this->displayonly != 'all')
		{
			...
			else
			{
				...
				$str .= $this->form->$submit_btn($this->submit_value, $this->submit_value, array('class' => $this->submit_class));
			}
		}
		...
		if (!empty($this->cancel_value))
		{
			...
			{
				$cancel_attrs = array('class' => 'button');
	...
	/**
	 * Render the HTML output search
	 *
	 * @access	public
	 * @param	array fields values... will overwrite anything done with the set_fields method previously
	 * @return	string
	 */
	public function render_divs_search($fields = NULL)
	{
		if (!empty($fields)) $this->set_fields($fields);
		...
		if (!empty($this->reset_value))
		{
			...
			{
				$str .= $this->form->reset($this->reset_value, '', array('class' => 'btn'));
			}
		}
		$str .= ' ';
		if (!empty($this->cancel_value))
	 ...
	 * @return	string
	 */
	public function render_public_search($fields = NULL)
	{
		if (!empty($fields)) $this->set_fields($fields);
		...
		$this->_html = '';
		$str = '<p>';
		$begin_str = '<div id="form_propiedades">';
		$end_str = '</div>';
		if ($this->display_errors)
		{
			$func = $this->display_errors_func;
	...
	/**
	 * Render the HTML output for bootstrap
	 *
	 * @access	public
	 * @param	array fields values... will overwrite anything done with the set_fields method previously
	 * @return	string
	...
	public function render_divs($fields = NULL)
	{
		...
		$this->_html = '';
		$str = '<fieldset>';
		$temp_str = '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">×</button><strong>Ups! ha ocurrido un error.</strong>{message}</div>';
		$begin_str = '';
		$end_str = '';
		if ($this->display_errors)
		{
			...
			if (function_exists($func))
			{
				if(strlen($func()) > 175)
				{
					$str .= str_replace('{message}', $func(), $temp_str);
				}
				else
				{
					$str .= $func();
		...
		if(!$this->display_modes)
		{
			...
			foreach($this->_fields as $key => $val)
			{
				...
				if ($val['type'] == 'section')
				{
					$str .= "<div class=\"section ".$val['class']."\">".$this->create_section($val)."</div>\n";
					continue;
				}
				else if (!empty($val['section']))
				{
					$str .= "<div class=\"section ".$val['class']."\"><".$this->section_tag.">".$val['section']."</".$this->section_tag."></div>\n";
					continue;
				}

				if ($val['type'] == 'copy')
				{
					$str .= "<div class=\"copy ".$val['class']."\">".$this->create_copy($val)."</div>\n";
					continue;
				}
				else if (!empty($val['copy']))
				{
					$str .= "<div class=\"copy ".$val['class']."\"><".$this->copy_tag.">".$val['copy']."</".$this->copy_tag."></div>\n";
					continue;
				}
				...
				else if (!in_array($val['name'], $this->exclude))
				{
					...
					$str .= $this->create_label($val, TRUE);
					$str .= "<div class=\"controls\">";
					if(!empty($val['before_html'])) $str .= "<div class=\"input-prepend input-append\">";
					$str .= $val['before_html'].$this->create_field($val, FALSE).$val['after_html'];
					if(!empty($val['before_html'])) $str .= "</div>";
					$str .= "</div></div>\n";
				}
			}
			$str .= "</div>\n";
		...
		else
		{
			...
			foreach($this->_fields as $key => $val)
			{
				...
				if ($val['type'] == 'copy')
				{
					$_str = "<div class=\"copy ".$val['class']." \">".$this->create_copy($val)."</div>\n";
					$temp = $_divs[$val['mode']];
					$temp .= $_str;
					$_divs[$val['mode']] = $temp;
					continue;
				...
				{
					$_str = "<div class=\"copy ".$val['class']."\"><".$this->copy_tag.">".$val['copy']."</".$this->copy_tag."></div>\n";
					$temp = $_divs[$val['mode']];
					$temp .= $_str;
					$_divs[$val['mode']] = $temp;
					continue;
				...
				if (!empty($val['custom']))
				{
					...
					if (!empty($this->row_id_prefix))
					{
						$_str .= ' id="'.$this->row_id_prefix.Form::create_id($val['name']).'"';
					}
					$_str .= " class=\"control-group\">\n";
					$_str .= $this->create_label($val, TRUE);
					$_str .= "<div class=\"controls\">\n";
					$_str .= $val['before_html'].$val['custom'].$val['after_html'];
					$_str .= "</div></div>\n";
					$temp = $_divs[$val['mode']];
					$temp .= $_str;
					$_divs[$val['mode']] = $temp;
				...
				else if (!in_array($val['name'], $this->exclude))
				{
					...
					$_str .= $this->create_label($val, TRUE);
					$_str .= "<div class=\"controls\">\n";
					if(!empty($val['before_html'])) $_str .= "<div class=\"input-prepend input-append\">";
					$_str .= $val['before_html'].$this->create_field($val, FALSE).$val['after_html'];
					if(!empty($val['before_html'])) $_str .= "</div>";
					$_str .= "</div></div>\n";
					$temp = $_divs[$val['mode']];
					$temp .= $_str;
					$_divs[$val['mode']] = $temp;
	...
	public function create_label($params, $use_label = TRUE)
	{
		...
		{
			$str .= "<label for=\"".Form::create_id($params['orig_name'])."\" id=\"label_".Form::create_id($params['orig_name'])."\" class=\"control-label\">";
		}
	...
	public function create_text($params)
	{
		...
		$attrs = array(
			'class' => $params['class'],
			'placeholder' => (!empty($params['placeholder']) ? $params['placeholder']: $params['label']),
			'original-title' => (!empty($params['original-title']) ? $params['original-title']: $params['label']),
			'data-provide' => (!empty($params['data-provide']) ? $params['data-provide']: ''),
			'data-item' => (!empty($params['data-item']) ? $params['data-item']: ''),
			'data-source' => (!empty($params['data-source']) ? $params['data-source']: ''),
	...
	public function create_select($params)
	{
		...
		$attrs = array(
			...
			'readonly' => $params['readonly'],
			'disabled' => $params['disabled'],
			'data-placeholder' => (!empty($params['data-placeholder']) ? $params['data-placeholder']: $params['label']),
			'original-title' => (!empty($params['original-title']) ? $params['original-title']: $params['label'])
		);
		$name = $params['name'];
	...
	public function create_section($params){
		...
		$str .= "<div class=\"controls\">";
		$str .= '<'.$this->section_tag.'>'.$section.'</'.$this->section_tag.'>';
		$str .= "</div>";
		$str .= "</div>\n";
		return $str;
	}
